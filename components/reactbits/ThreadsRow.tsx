...
const ThreadsRow: React.FC<ThreadsProps> = ({
  color = [1, 1, 1],
  amplitude = 1,
  distance = 0,
  className,
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uAmplitude: { value: amplitude },
        uDistance: { value: distance },
        uMouse: { value: new Float32Array([0.5, 0.5]) }, // 중앙 고정
      },
    });
    ...
    // ✅ 화면에 보일 때만 실행: IntersectionObserver
    const io = new IntersectionObserver(
      (entries) => {
        const entry = entries[0];
        isVisibleRef.current = !!entry?.isIntersecting;
        ...
        if (isVisibleRef.current && !isRunningRef.current) {
          isRunningRef.current = true;
          animationFrameId.current = requestAnimationFrame(update);
        } else if (!isVisibleRef.current && isRunningRef.current) {
          isRunningRef.current = false;
          if (animationFrameId.current)
            cancelAnimationFrame(animationFrameId.current);
        }
    ...
    const onVisibilityChange = () => {
      if (document.hidden) {
        isVisibleRef.current = false;
        if (isRunningRef.current && animationFrameId.current) {
          cancelAnimationFrame(animationFrameId.current);
          isRunningRef.current = false;
        }
      } else {
